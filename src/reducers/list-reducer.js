import { FILTER_LIST, ADD_TO_LIST, UPDATE_TO_LIST, DELETE_FROM_LIST } from '../actions/list-actions';
...
export default function (state=initialState, action) {
  switch (action.type) {
    case FILTER_LIST: {
      return {
        ...state,
        items: state.filter(item => item.wishId === action.payload.wishId)
      }
    }
    ...
    case UPDATE_TO_LIST: {
      return {
        ...state,
        lists: state.map(item => item.id === action.payload.id ? action.payload : item)
      }
    }
    ...
    case DELETE_FROM_LIST: {
      return {
        ...state,
        lists: state.filter(item => item.id !== action.payload.id)
      }
    }
